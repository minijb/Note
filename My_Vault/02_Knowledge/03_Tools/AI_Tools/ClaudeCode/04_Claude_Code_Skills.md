...
### Skill 与 Agent 的对比
虽然两者都可以扩展 Claude Code 的能力，但它们的设计初衷和工作方式有显著区别：
...
| **触发机制** | 绝大多数情况通过上下文**自动触发**（根据 `description` 匹配），少部分通过 `/` 命令手动触发。 | 通过工具调用（Tool call）或者在后台作为子进程生成，通常用于处理复杂的多步任务。 |
| **适用场景** | 确保代码符合特定框架规范（如 Spring Boot 安全）、约束解释风格、提供特定领域的背景知识。 | 深入代码库进行开放式探索、执行需要多次尝试和验证的重构、自主收集和整理大量信息。 |
