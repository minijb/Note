...
aliases:
  -
description: [RequireComponent(typeof(RectTransform))]
draft: false
---
	...
	public class AdvancedInputField: Selectable, IPointerClickHandler, IPointerExitHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IUpdateSelectedHandler
	{
		...
		public TextRenderer TextRenderer
		{
			get
			{
				...
				if(textRenderer == null)
                {
                    ...
                    var text = transform.Find("TextArea/Content/Text");
		...
		public TextRenderer ProcessedTextRenderer
		{
			get
			{
				...
				{  
                    if (transform == null) return null;
                    var p = transform.Find("TextArea/Content/Processed");
                    if (p == null) return null;
					processedTextRenderer =p.GetComponent<TextRenderer>();
				}
		...
		public TextRenderer PlaceholderTextRenderer
		{
			get
			{
				...
				{
                    if (transform == null) return null;
                    var p = transform.Find("TextArea/Content/Placeholder");
                    if (p == null) return null;
                    placeholderTextRenderer = p.GetComponent<TextRenderer>();
				}
        ...
        public bool IsKeyBoardShow()
        {
            var state = Engine?.KeyboardClient?.Keyboard.State;
            return state == KeyboardState.PENDING_SHOW || state == KeyboardState.VISIBLE;
        }
...
类继承自 `Selectable` 并实现了多个接口，包括 `IPointerClickHandler`、`IPointerExitHandler`、`IBeginDragHandler`、`IDragHandler`、`IEndDragHandler` 和 `IUpdateSelectedHandler`，用于在 Unity 中管理和操作高级输入字段。该类包含多个字段和方法，用于处理输入字段的初始化、文本编辑、光标位置、文本选择、事件触发等功能。

类中定义了多个事件类，包括 `SelectionChangedEvent`、`BeginEditEvent`、`EndEditEvent`、`ValueChangedEvent`、`CaretPositionChangedEvent`、`TextSelectionChangedEvent`、`SizeChangedEvent`、`SpecialKeyPressedEvent`、`TextTapEvent` 和 `ActionBarActionEvent`，用于在特定操作发生时触发相应的事件。每个事件类都继承自 `UnityEvent`，并接受不同类型的参数，例如布尔值、字符串、整数和自定义类型。

在 `AdvancedInputField` 类中，还定义了多个序列化字段，用于在 Unity 编辑器中配置输入字段的属性。这些字段包括输入字段模式、文本渲染器、占位符文本渲染器、光标渲染器、文本选择渲染器、文本内容、占位符文本、富文本编辑、字符限制、行限制、内容类型、行类型、输入类型、密码可见性、键盘类型、字符验证、表情符号允许、富文本绑定、实时处理过滤器、实时装饰过滤器、后处理装饰过滤器、选择模式、拖动模式、光标闪烁率、光标宽度、光标颜色、选择颜色、选择在文本后面、只读模式、结束编辑时的滚动行为、滚动条可见性模式、滚动速度、最大滚动过渡时间、快速滚动灵敏度、最小宽度、最大宽度、最小高度和最大高度。

此外，`AdvancedInputField` 类还包含多个属性和方法，用于获取和设置输入字段的各种属性，例如文本内容、光标位置、文本选择起始位置和结束位置、富文本内容、占位符文本、是否有选择、选中文本、渲染文本、是否启用操作栏、是否启用触摸选择光标、光标夹紧模式、下一个输入字段等。类中还定义了多个 Unity 生命周期方法，包括 `Awake`、`OnEnable`、`OnDisable`、`OnDestroy`、`Update`、`LateUpdate` 和 `OnRectTransformDimensionsChange`，用于在特定生命周期阶段执行相应的操作。

在 `AdvancedInputFieldEditor` 类中，定义了多个序列化属性，用于在 Unity 编辑器中显示和编辑 `AdvancedInputField` 类的属性。该类继承自 `SelectableEditor`，并重写了 `OnEnable`、`OnDisable` 和 `OnInspectorGUI` 方法，用于初始化和更新编辑器界面。类中还定义了多个私有方法，用于绘制不同类别的属性，例如继承属性、模式属性、外观属性、常规属性、处理属性、事件属性、其他属性和移动属性。

在 `ConversionToolWindow` 类中，定义了多个方法，用于查找和转换不同格式的 `AdvancedInputField` 对象。类中包含多个方法，例如 `GetAdvancedInputFieldsUnityText`、`GetAdvancedInputFieldsTextMeshProText` 和 `GetDeprecatedAdvancedInputFields`，用于查找特定格式的 `AdvancedInputField` 对象。类中还定义了 `TryCopyComponents` 方法，用于将组件从一个游戏对象复制到另一个游戏对象。

在其他文件中，`AdvancedInputField` 类被多次引用和使用。例如，在 `FormView` 类中，定义了多个 `AdvancedInputField` 字段，用于存储表单中的输入字段。在 `MultiTextSelectionHandler` 类中，定义了一个 `AdvancedInputField` 字段，并在 `Awake` 方法中初始化该字段。在 `RegisterView` 类中，定义了多个 `AdvancedInputField` 字段，用于存储注册表单中的输入字段。在 `KeyboardScroller` 类中，定义了多个方法，用于处理键盘滚动和输入字段的滚动行为。

总体来说，`AdvancedInputField` 类提供了丰富的功能，用于在 Unity 中管理和操作高级输入字段，包括文本编辑、光标位置、文本选择、事件触发等功能。通过这些方法和属性，可以方便地在 Unity 中实现复杂的输入字段操作，提高用户交互体验。