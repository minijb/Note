---
title: ch mermaid 00 List
date: 2026-03-16
tags:
  - resource
type: knowledge
aliases:
  -
description: - [[ch_mermaid_01_流程图]]
draft: false
---

# ch mermaid 00 List

- [[ch_mermaid_01_流程图]]
- [时序图](https://mermaid.nodejs.cn/syntax/sequenceDiagram.html)
- [类图](https://mermaid.nodejs.cn/syntax/sequenceDiagram.html)
- [状态图](https://mermaid.nodejs.cn/syntax/sequenceDiagram.html)
- [甘特图](https://mermaid.nodejs.cn/syntax/gantt.html)
- [思维导图](https://mermaid.nodejs.cn/syntax/mindmap.html)
- [ZenUML/时序图](https://mermaid.nodejs.cn/syntax/zenuml.html)
- [数据包图](https://mermaid.nodejs.cn/syntax/packet.html)
- [示例](https://mermaid.nodejs.cn/syntax/examples.html)

