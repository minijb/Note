...
可以通过子类来继承父类的方法，同时相同的子类中方法对覆盖父类中(重写)的方法

我们可以将他们的实例放在同一个列表中来同一运行共有的方法
...
这是c不关注类是谁，只要有这个方法就会去运行

在其他应用中，多态可以隐藏接口的差异性。如处理数据流的程序是预期给有输入和输出方法的对象编写的，但是并不关心方法实际在做什么

1

```
def processer(reader , converter , writer):
```
...
```
processer(FileReader(),Converter,xxx)
```

19

```
processer(SocketReader(),Converter,xxx)
```

## 21 类基础

- class语句创造类对象并赋值给一个名称
- class语句内的赋值语句会创建类的属性
- 类属性提供了对象的状态和类型

**当调用对象时，我们得到了实例对象，对象有一下特点**

- 想函数一样调用类会创建新的实例对象
- 每个实例基础了类的属性并获得了自己的命名空间
- 在方法内对self属性赋值运算会产生每个实例自己的属性
...
x,y都继承了First类中的方法，但是因为设置的data属性不同，所以显示的也不同
...
类名称没有什么特殊的，同样可以通过import进行导入等操作

### 类可截获运算符

- 以双下划线命名的是特殊的钩子函数`__X__`
- 当实例出现在内置运算中时，类方法会自动被调用:如兑现继承了`__add__`方法，那么当对下出现在+表达式中就会自动调用方法，该方法的返回值会作为相应的表达式的结果
- 类可以重置绝大多数内置类型运算
- 默认的运算符重载方法及不存在也不需要：如没有add方法，那么在+表达式中，就会出现异常

例子

- `__init__`会在创建新的实例时被调用
- `__add__`会在+表达式中的时候运行
- `__str__`会在打印对象的时候被调用
...
```
print(a)#THRID : abcabcabc
```

**通过dict可以查看类或者对象的命名空间**

1

```
list(Thrid.__dict__.keys())
```

2

```
#['__module__', '__init__', '__add__', '__str__', 'mul', '__doc__']
```

**在创建实例的时候，实例的命名空间是空的，因为那些属性都是继承来的，但是如果修改实例的属性，那么就是在实例中重写了属性，不是在类中修改**
...
```
list(a.__dict__.keys())#['data']
```

13

```
#在初始化时修改了属性所以有data没有q
```
...
```
list(a.__dict__.keys())#['data', 'q']
```

**为了完成属性的继承搜索，每个实例都拥有一个`__class__`连接**

1

```
print(a.__class__)#<class '__main__.Thrid'>
```

**`__base__`是其类的父类对象引用的元组**

1

```
print(Thrid.__bases__)
```
...
而`__getattr__`能拦截未定义属性的访问并把这些访问委托给getattr内置函数调用内嵌的对象
...
```
        self.person=Person(name,pay)
```

15

```
    def getRaise(self,percent,bonus=.10):
```
