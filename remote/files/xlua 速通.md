...
tags:
  ...
  - lua
---
插件 ： emmylua  -- 设置 `.lua.txt` 进行扩展。

## 如何调试

ridder ， 需要copy 部分代码 并包装到一个组件中。
...
luaEnv.DoString(xxx, xx)

// 获取函数

Action xx = scriptScopeTable.Get<Action>(key);
scriptScopeTable.Get(key, out xxxx);
